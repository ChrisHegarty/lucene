...
public abstract sealed class Lucene99MemorySegmentFloatVectorScorerSupplier
  ...
  static Optional<RandomVectorScorerSupplier> create(
    ...
    return switch (type) {
      ...
      case MAXIMUM_INNER_PRODUCT -> Optional.empty(); //of(new MaxInnerProductSupplier(msInput, values));
    };
  }
    ...
    public UpdateableRandomVectorScorer scorer() {
      return new UpdateableRandomVectorScorer.AbstractUpdateableRandomVectorScorer(values) {
        ...
        public float score(int node) throws IOException {
          ...
          MemorySegment ms1 = getSegment(queryOrd, scratch1);
          // TODO: wasteful?
          PanamaVectorUtilSupport.dotProductBulkFromSegments(scratchScores, query, ms1, ms1, ms1, ms1, dims);
          return Math.max((1 + scratchScores[0]) / 2, 0);
        }
        ...
        public void scoreBulk(float[] scores, int node1, int node2, int node3, int node4)
          ...
          MemorySegment ms4 = getSegment(node3, scratch4);
          PanamaVectorUtilSupport.dotProductBulkFromSegments(scores, query, ms1, ms2, ms3, ms4, dims);
